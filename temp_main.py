...
class AppController:
    ...
    def _on_file_selected(self, path):
        ...
        self.window.data_editor_tab.lbl_project.setText(f"Editing Project: {project}")
    ...
    def _load_config(self):
        ...
        st.translation_style_cb.setCurrentText(prefs.get("translation_style", "Standard (┘╪╡╪ص┘ë)"))
    ...
    def _start_translation(self):
        ...
        trans_style = self.config_cache.get("preferences", {}).get("translation_style", "Standard (┘╪╡╪ص┘ë)")
    ...
    def _resume_translation(self):
        ...
        self._log_internal("Resuming translation...", "╪ش╪د╪▒┘è ╪د╪│╪ز╪خ┘╪د┘ ╪د┘╪ز╪▒╪ش┘à╪ر...")
        self.runner.start(
            file_path=path,
            provider=provider,
            api_key=api_key,
    ...
    def _stop_translation(self):
        self._log_internal("Translation stopped forcefully.", "╪ز┘à ╪ح┘è┘é╪د┘ ╪╣┘à┘┘è╪ر ╪د┘╪ز╪▒╪ش┘à╪ر ╪ح╪ش╪ذ╪د╪▒┘è╪د┘ï.")
        ...
        if state in ["Idle", "Completed", "Failed"]:
            ...
            if hasattr(self, 'batch_queue') and self.batch_queue and (state == "Completed" or state == "Failed"):
                ...
                if self.batch_queue:
                    ...
                    self._log_internal("\nAuto-advancing to next file in queue...\n", "\n╪د┘╪د┘╪ز┘é╪د┘ ╪د┘╪ز┘┘é╪د╪خ┘è ┘┘┘à┘┘ ╪د┘╪ز╪د┘┘è ┘┘è ╪د┘╪╖╪د╪ذ┘ê╪▒...\n")
